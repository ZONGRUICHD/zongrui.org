...
| 样式 | 手写 CSS、CSS Custom Properties、Grid、Flex、响应式媒体查询、`prefers-reduced-motion`；没有使用 Tailwind、Sass 或组件 UI 框架 |
...
TipTap 正文支持 H2/H3、粗体、斜体、下划线、删除线、引用、列表、链接、分隔线、代码块、带图注图片及撤销/重做。H1 只用于文章标题，不允许在正文中重复出现。
...
文章支持草稿、定时发布、已发布和归档状态；支持题图、摘要、标签、可编辑 slug、发布/撤回、修订恢复和一级评论。评论只按纯文本显示，可隐藏、恢复或软删除。
...
后端当前有 16 项 pytest 测试，覆盖文章状态、搜索、ETag、revision 冲突、修订恢复、slug 重定向、富文本限制、OAuth、CSRF、Turnstile、评论限流与审核、图片重编码、定时发布和种子数据。`pytest-cov` 用于覆盖率检查，Ruff 用于 Python 静态检查。
...
网站文案来自公开项目资料与用户明确授权的 ChatGPT 账号记忆摘要，并经过隐私筛查。仓库不保存 ChatGPT 对话、联系方式、学校信息或其他私密资料。
