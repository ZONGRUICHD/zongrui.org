...
ZongRui 的个人网站，包含交互式个人主页、独立技术项目档案、公开图片展、网页存档、GitHub/Codex 活动墙，以及一套自托管的完整博客系统。

- 主站：[https://zongrui.org](https://zongrui.org)
- 博客：[https://zongrui.org/articles](https://zongrui.org/articles)
- 技术作品：[https://zongrui.org/projects](https://zongrui.org/projects)
- 图片：[https://zongrui.org/gallery](https://zongrui.org/gallery)
- 统一管理后台：[https://zongrui.org/console](https://zongrui.org/console)
...
| UI | React 19.2.7、React DOM 19.2.7、React Hooks；Pixel OS 式自适应 App Shell；官方 `@material/web` 2.5.0 Web Components 提供按钮、图标按钮、涟漪、文本框、筛选 Chip 与选择器 |
| 设计系统 | Material You / Material 3；官方 `@material/material-color-utilities` 0.4.0 根据樱粉、Pixel 蓝、鼠尾草和日落四种壁纸种子生成浅色与深色 `--md-sys-color-*` 角色，并补齐新版 surface-container 角色 |
...
| 系统交互 | 手机状态栏与下拉快捷设置、导航栏/手势条、平板与桌面导航轨、应用抽屉、Snackbar、动态壁纸取色、亮度遮罩、高对比度、触觉反馈与 View Transitions 页面转场 |
| 样式 | 基于官方 Material 3 tokens 的 Pixel OS 自适应布局、CSS Custom Properties、Grid、Flex、响应式媒体查询、可持久化的系统/浅色/深色主题与 `prefers-reduced-motion`；没有使用 Tailwind 或 Sass |
| Web App | `manifest.webmanifest`、standalone/window-controls-overlay 显示模式、动态 `theme-color`、安全区域与移动端系统栏适配 |
...
| GitHub OAuth 边缘中继 | 受共享密钥保护的 Worker 端点完成 token exchange，只向后端返回管理员数字 ID、login 与头像，不向浏览器暴露令牌 |
| Workers Cache API | 以缓存代际隔离公开文章、图片、标签、RSS 和 sitemap；源站短暂离线时提供 stale 副本 |
...
- 文章详情新鲜时间 5 分钟。
- 文章列表、图片列表、标签、RSS 和 sitemap 新鲜时间 1 分钟。
- 搜索、筛选和 cursor 分页使用包含完整查询参数的独立缓存键；写操作推进缓存代际，使所有旧查询组合同时失效。
- 评论、鉴权、Console 和管理请求统一 `no-store`。
- 首页/文章访问统计的 GET 与 POST 统一 `no-store`，不进入边缘内容缓存。
- 源站返回 `429` 或 `5xx` 时，公开内容最多可使用 7 天 stale 副本。
- 管理端写入后使用 `X-ZR-Cache-Invalidate` 推进缓存代际，失效文章、图片、列表、标签、RSS 和 sitemap 的旧副本。
...
| HTTP 客户端 | HTTPX，用于受保护的 GitHub OAuth 边缘中继与 Turnstile 服务端验证；无中继配置时保留直连回退 |
| AI 辅助翻译 | Console 可按需调用 DeepSeek `deepseek-v4-flash`，将标题、摘要、正文及图片说明从简体转为繁体；严格保留 TipTap 结构、代码、链接和媒体排版，API Key 仅存服务器 |
| 富文本安全 | TipTap JSON 节点/mark 白名单、Bleach HTML 二次清洗、纯文本提取与中英文阅读时间计算 |
| 图片管线 | Pillow；文章媒体与公开图片展共用 JPEG/PNG/WebP 验证、EXIF 方向处理、最长边缩至 2000px、去元数据、统一转 WebP |
...
| 访问统计 | 首页网络访客估算、逐文章阅读估算；独立 HMAC 密钥、IPv6 `/64` 归一、按站点/文章隔离摘要；不存原始 IP、UA、路径或逐访客时间 |
...
统一 `/console` 同时管理首页简介、文章、评论和公开图片。首页简介保存为后端单例配置，公开首页只读获取，并按文字长度自动切换字号；图片先保存为草稿，可补充标题、说明、替代文字和顺序，再发布或归档。公开 `/gallery` 永远只读取已发布记录。旧 `/articles/console*` 地址由边缘 308 到等价的新地址，已有书签和 OAuth 回跳仍可使用。
...
- DeepSeek 翻译只在管理员于 Console 主动触发时发送待转换文字；API Key 由后端用于鉴权且不会下发浏览器，代码块、媒体二进制和图片地址不会进入 DeepSeek 翻译载荷。
...
| `/projects`、`/projects/:slug` | 浏览器 → Pages 深链 shell → React 项目索引与实现档案 |
| `/gallery` | 浏览器 → Pages 深链 shell → 同源公开图片 API → 内容寻址 WebP |
| `/console/*` | 管理员 → 私有 no-store/noindex shell → GitHub OAuth → 首页简介、文章、评论与图片管理 API |
| `/api/articles/*` | 浏览器 → Pages Function → Access service token → Tunnel → FastAPI |
| 首页与文章阅读统计 | 页面可见并停留 3 秒 → 同源统计 POST → 分用途网络摘要去重；文章 POST 同时登记全站与文章计数 |
| Console 简体转繁体 | 管理员 → 同源管理 API + CSRF → FastAPI 分段校验 → DeepSeek JSON Output → TipTap 结构复验 → 编辑区预览/修订保存 |
...
```text
.
├─ src/                         React 前端、Pixel OS / Material You 系统壳、文章、项目、图片、Console 与活动墙组件
│  ├─ pixel/                    状态栏、导航、抽屉、快捷设置、系统图标与交互状态
...
后端当前有 84 项 pytest 测试，覆盖首页简介读写与鉴权、文章状态、置顶排序与分页、搜索、ETag、revision 冲突、修订恢复、slug 重定向、排版与内容语言、公开图片展及安全重编码、OAuth 回跳约束、CSRF、Turnstile、评论限流与审核、DeepSeek 翻译映射与失败边界、定时发布、种子数据，以及统计去重、隐私信号、跨站拒绝、IPv6 归一和并发写入。另有 15 项 Node 测试验证 OAuth 边缘中继、统一 Console 与旧地址重定向、项目/图片深链元数据、真实 404、动态 sitemap、静态资源回退、首页简介、文章和图片的边缘缓存及跨查询缓存失效。`pytest-cov` 用于覆盖率检查，Ruff 用于 Python 静态检查。
...
评论仅保存昵称和正文；不收集邮箱或网址。活动墙只保存按天聚合的贡献、turn 和 token 统计，不保存 Codex 对话正文。访问计数按公网网络估算：服务端不保存原始 IP、UA 或浏览路径，只长期保留按站点/文章隔离的加密去重摘要，直到计数器被重置；共享网络会合并，动态地址可能重复，因此数字明确标为估算。
