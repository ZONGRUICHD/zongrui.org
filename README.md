...
后端当前有 38 项 pytest 测试，覆盖文章状态、搜索、ETag、revision 冲突、修订恢复、slug 重定向、横排/直排、富文本限制、OAuth、中继地址约束、CSRF、Turnstile、评论限流与审核、图片重编码和排版、定时发布、种子数据，以及统计去重、隐私信号、跨站拒绝、IPv6 归一和并发写入。另有 4 项 Node 测试验证 OAuth 边缘中继的鉴权边界、回调绑定、错误分类和令牌不泄露。`pytest-cov` 用于覆盖率检查，Ruff 用于 Python 静态检查。
